...
**Статус:** проектирование завершено (Этап 1), разработка не начата.

## Архитектура (коротко)

- **Экземпляр клиента:** Spring Boot 4.1 (Java 25 LTS) + PostgreSQL 18 (+pgvector) + Garage (S3) + Angular 22.
  Бизнес-логика — в приложении; в БД — целостность и аудит. Без multi-tenancy: изоляция физическая.
- **Control plane (наш, один):** реестр клиентов, версии/heartbeat, лицензии, объявления,
...
| Документ | Что внутри |
|---|---|
| [ТЗ-01: CMS](docs/trd/TRD-01-cms.md) | Требования Этапа 1: экземпляр + control plane |
