...
- Modify `config/security/RateLimitFilter.java` only to translate initial policy-unavailable `ApiException(SERVICE_UNAVAILABLE)` during search-budget resolution into a fixed safe503 through the existing problem writer; do not catch other exception codes or alter unrelated route/anonymous budgets. Add a real HTTP regression proving no engine work and preserved unrelated paths.
- Create tests under `apps/server/src/test/java/com/greenwhite/dwh/instance/search/`: `SearchSettingsIntegrationTest.java`, `SearchManagementAuthorizationTest.java`, `SearchStatusTest.java`; extend HTTP/search/rate tests from package 1 and adjust the existing `SecurityConfigTest` MVC fixture for the real saved-policy provider dependencies.
...
- [ ] Add sanitized Typesense health/debug/metrics/schema metadata adapters. Status separates dependency health, initialization, rebuild-required, queue lag/failed deliveries and last successful reconciliation. Return counts and timestamps, not full documents. Health failure yields nullable unknown index counts, not zero. Surface effective connect/read/fallback budgets and clamped cookie/API search rate; expose dependency version but not configured URL/key. Disk metrics from `/metrics.json` are installation-wide, not per-generation storage. Show per-generation document count and nullable storage size when not provided; do not label global disk usage as a generation's bytes.
  Preserve the design's entity-level breakdown inside each generation as typed TASK/PROJECT/USER counts as well as the aggregate. A collection whose count is unavailable has its own null; preserve other known counts, distinguish valid zero, and expose no physical names. Test distinct entity counts and one unavailable collection through the real HTTP metadata/status boundary.
...
An omitted `limit` on the normal GET endpoint resolves to the saved global limit, not a hardcoded controller10. Explicit valid caller limits remain bounded by that policy. Add a real controller/consumer regression that saves a limit above10, requests search without `limit`, and observes the saved budget in returned results/emitted engine parameters. Keep the unchanged default policy behavior and reject invalid explicit limits. The palette from the reliability package omits this parameter by default.
...
**Files:**
- Create `apps/server/src/main/resources/db/migration/V027__search_job_request_metadata.sql` and search test `SearchJobMetadataMigrationTest.java`; add nullable `requested_generation_id uuid`, nullable `retry_of_job_id uuid references search_jobs(id)` and `request_metadata_recorded boolean not null` without modifying V026 or its constraints/indexes. Add the marker with default false for existing rows, then change its default to true for new rows. Verify upgrade from V026 with existing job rows, both marker values and repeat migration; adjust existing latest-migration/count expectations narrowly where V027 requires it. Retry creates a new linked job/request ID while reusing the generation, preserving the original request replay/history. Do not invent missing historical original-target metadata: replay of an old marker-false job returns safe 409 `REQUEST_HISTORY_UNAVAILABLE`; its history remains readable and a new explicit retry can use its known resolved generation.
...
- Modify `apps/server/src/test/java/com/greenwhite/dwh/instance/audit/AuditCoverageTest.java` narrowly to recognize the explicit `SearchJobService` → `SearchJobRepository` audit dependency as a separate checked mapping. Assert that the mapped service exists and actually depends on that repository; do not add it to the no-audit exemptions or accept arbitrary repository dependencies. Retain real PostgreSQL start/cancel/retry/explicit-actor switch audit and audit-failure atomicity coverage. The worker's audit uses the existing immutable storage on its transaction connection, not a forged request SecurityContext or an unused `AuditLogService` injection.
...
- Add an `AutoCloseable` metadata cursor in `TypesenseDocumentStream`, opened through `TypesenseClient`, with `readPage(maxRows, maxBytes)` bounded to at most 100 rows and 1 MiB encoded bytes; retain `forEachDocumentMetadata` as a cursor-backed compatibility wrapper. Distinguish explicit EOF from an empty yielded page, bound a single line before full allocation, and report oversize/malformed/timeout errors safely. Only one job-owned cursor and TEMP-comparison connection may survive between VERIFYING cycles; no open DB transaction spans HTTP. Resume bounded export/source-comparison units across coordinator cycles, close resources on cancel/failure/ownership loss/shutdown, and invalidate/restart proof on process restart. A timeout must bound blocking reads so active deliveries and shutdown cannot be starved by a stalled export; cleanup may touch only the job-owned temporary proof objects.
...
- [ ] Implement server-generated physical names `cms_<uuidWithoutDashes>_tasks/projects/users`, frozen schema-defining snapshot (`schema_version`, `schema_profile`, originating `settings_version`), and keyset discovery through the shared reader/revision queue. These persisted generation values reproduce the fixed schema after restart; weights, prefix, typo and rate settings remain live query settings, not a historical policy copied into each generation. Generation name components never come from user strings. Existing legacy names remain registered until verified cutover. Rebuild never clears active index. Add metadata fields and RU locale/stem only on intended text fields in a new RU generation; keep login/email/phone exact-identifier tokenization. Collection creation only follows verified absence, not every HTTP error.
