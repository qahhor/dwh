...
**Files:**
- Create `apps/server/src/main/resources/db/migration/V027__search_job_request_metadata.sql` and search test `SearchJobMetadataMigrationTest.java`; add nullable `requested_generation_id uuid` and nullable `retry_of_job_id uuid references search_jobs(id)` without modifying V026 or its constraints/indexes. Verify upgrade from V026 with existing job rows and repeat migration; adjust existing latest-migration/count expectations narrowly where V027 requires it. Retry creates a new linked job/request ID while reusing the generation, preserving the original request replay/history. Do not invent missing historical original-target metadata.
