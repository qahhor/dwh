...
- Keep scoped list/detail `UrlMatcher` helpers with the typed route mapping and add `core/guards/record-navigation.guard.ts` plus focused tests for the task/project page-leave contract. Each entity uses one route config for list/detail, preserving component-local filters. Keep existing `/tasks` list navigation compatible alongside `/tasks/items` and `/tasks/items/:id`; project/user routes stay exact, with project matching before the task family. Reuse existing discard dialogs for route-param and cross-page navigation, settle cancelled/replaced guard requests safely, and prove preservation through the actual router. Do not install a global RouteReuseStrategy.
...
Canonical route IDs stay strings through GET URLs, request-generation guards and displayed identity. For a route ID whose legacy numeric DTO cannot be represented safely, keep details readable and comments GET canonical, but disable numeric-dependent mutations/editor handoff/subtask actions and unsafe related-ID navigation. Safe IDs retain existing permission-gated flows. This is a scoped compatibility boundary, not a global DTO serialization change; test a real rounded-number response fixture alongside its exact route key and ensure no action can target the rounded value.
