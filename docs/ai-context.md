...
### Точка продолжения — 2026-09-26 (low-code: волна 9)

Перенос UI-кита завершён: волны 7–8 и п. 40 роадмапа слиты или ждут слияния,
`ui-button` и `ui-modal` удалены — кнопки `smt-button`, диалоги `smt-dialog`
(ADR-0015, записи 45–54). Роадмап — артефакт Claude Docs «Роадмап»
(таблица волн, статус меняется выпадающим списком в последней колонке).

**Low-code.** [ADR-0019](adr/ADR-0019-low-code-entity-model.md) принят
2026-09-26 (вариант A: сущность объявляется метаданными в коде, поверх —
безопасный слой администратора, сущности без кода — отдельным решением).
Волна 9 (пп. 47–58) по [ADR-0016](adr/ADR-0016-field-registry-query-dsl.md):

| Пункт | Состояние |
|---|---|
| 47 меню по правам (`FR-MOD-02`) | в `main` |
| 48 пользователи `iam.users` | ветка, ждёт слияния |
| 49 задачи `ms.tasks` | ветка |
| 50 аудит `audit.logs`, `audit.security_events` | ветка |
| 51 заметки `ms.notes`; проекты | заметки — ветка; проекты отложены до пп. 53/56 (сортировка по прогрессу зависит от скоупа зрителя) |
| 52 дополнительные поля как поля реестра | ветка |
| 53 ссылки `ref` и группы «или» | следующий |
| 54–58 | не начаты |

Ветки идут стопкой: `claude/lowcode-users-registry` → `-tasks-registry` →
`-audit-registry` → `-projects-notes` → `-custom-fields-registry` →
`claude/maintenance-tests-cleanup`. Отдельно — `claude/hotfix-user-roles`
(исправление ролей для `main`). `gh` не авторизован: PR создаются по
compare-ссылкам вручную.

**Правила реестра, выученные в этой волне.**

- Прежние плоские параметры списков сохраняются (`search` — синоним `q`) и
  входят в отпечаток курсора: `QueryCompiler.compile(..., narrowing)`.
- Экспортёр проверяет значения опций до запуска задания
  (`QueryListExporter.checkOptions`).
- Составной порядок выражается скрытым текстовым полем сортировки
  (`ms.notes.rank`: закреплённые, затем время изменения).
- Журнал аудита секционирован по времени: сортируется только время события.
- Дополнительные поля добавляются к списку на каждый запрос
  (`QueryListRegistry.resolve` + `QueryListExtender` в `md`); они не
  сортируются — у рабочей роли БД нет прав на DDL для индекса по выражению.
- На вебе список грузит `query-meta` один раз, затем виды
  (`ListViewState`), затем первую страницу; тестовые метаданные — в
  `src/testing/registry-meta.ts`.

**Найденные и исправленные дефекты:** `MdRoleRepository.getUsersRoleIds` не
выполнял запрос (роли в списке пустые; правка из списка снимала все роли) —
хотфикс; `required_permission` пунктов меню нигде не применялся; правка пункта
меню стирала его право и родителя; изменения вариантов и значения по умолчанию
дополнительного поля не попадали в аудит.

**Тесты.** Серверные интеграционные тесты без Spring переводятся с
Testcontainers на общий встроенный PostgreSQL:
`TestDatabases.migratedCopy(prefix)` копирует шаблонную БД с миграциями и
отдаёт маленький пул Hikari. На Windows каждое новое соединение со встроенным
сервером запускает процесс, поэтому соединение на запрос
(`DriverManagerDataSource`) делало такие тесты в разы медленнее.
`SearchJobResourceIntegrationTest` нестабилен под нагрузкой полного прогона
(фиксированное число шагов `runOnce`); вынесено отдельной задачей.

**Ловушки окружения.** Bash-инструмент ломает `$`, обратные кавычки и `\` во
встроенных скриптах — файлы писать через Write; исходники в CRLF.
`signal-order-audit --fix` переставляет поля класса: поле, которое читает
другое при инициализации (`this.destroyRef`), должно остаться выше.
...
убраны, значения состояния сохранены. `UiModal` (с 2026-09-26 заменён
`smt-dialog` кита) потреблял обработанный
