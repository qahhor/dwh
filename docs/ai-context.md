...
### Точка продолжения — приостановлено по запросу пользователя

2026-09-08 пользователь попросил сохранить контекст. HEAD —
6c2e7743234b964a01580090e8b758c37ec633c8, main впереди origin/main на 18
коммитов. Push/deploy пакета не выполнялись; localhost:4200 не изменялся.

Шесть этапов реализации и поэтапные ревью завершены. Общее ревью нашло
обязательный НЕ исправленный дефект: Users/Roles уничтожают панель при
отзыве iam.org_units.view во время PUT и преждевременно снимают pending.
Нужно сохранить владельца запроса до ответа, скрыть защищённые данные и
проверить реальные hosts при revoke/restore и success/error. Отмена
клиентской подписки не доказывает остановку серверной транзакции.

В единую завершающую корректировку также включены: нормализованное dirty-
сравнение имени; удаление заведомо удалённого узла из локального выбора даже
при refresh error; missing-assignment warning только после успешного tree
read; точный method/path/count учёт контролируемого 503.

Корректировка начата, но нового коммита и отчёта о тестах нет. Сохранены
новые dirty изменения пяти IAM тестов: org-unit-editor.component.spec.ts,
org-units.component.spec.ts, user-org-units-panel.component.spec.ts,
roles.component.spec.ts и users.component.spec.ts. Сначала проверить diff;
результат RED/GREEN для этих правок пока НЕ установлен. Активного исполнителя
при сохранении нет; final-fix-report.md ещё отсутствует.

План: [оргструктура](superpowers/plans/2026-09-08-org-structure-ui.md).
Локальная точка исполнения: .superpowers/sdd/2026-09-08-org-structure-ui/.
Прочитать progress.md и final-fix-brief.md, продолжить прерванную единственную
финальную волну, затем проверить окончательный source и выполнить одно
scoped re-review. Завершённые этапы не повторять, частичные тесты не откатывать.

Прежние семь dirty UI-файлов, Graphify и пять audit-черновиков сохраняются
отдельно. Вопрос о включении существующего shared-UI фикса контраста отдельным
локальным коммитом остался БЕЗ ОТВЕТА; разрешение на staging этих изменений,
push или deploy не получено.

Подтверждённые результаты ниже относятся к коду ДО новой корректировки:
Maven 750/750; clean Angular 492/492; preserved-UI Angular 501/501;
clean browser 6/7 (контраст 2.1423:1), preserved-UI browser 7/7. Полная clean-
source приёмка открыта. Синтетический проект smartupcms-org-ui-20260908 на
127.0.0.1:14208 не удалён, последний проверенный web — eefd8ae-preserved-ui.
Артефакты: C:/Temp/smartupcms-org-structure-qa-20260908/. При продолжении заново
проверить Git status, текущие разрешения и фактическое состояние стенда.
