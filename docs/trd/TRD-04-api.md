...
├── not_found                       # Узел оргструктуры не найден
├── conflict                        # Перенос узла под собственного потомка (I-ORG-1);
│                                   # удаление узла с детьми или сотрудниками (I-ORG-2);
│                                   # попытка создать второй корень дерева
└── validation_failed               # Неизвестное правило видимости (допустимы ALL, SUBTREE, UNITS, SELF)
```

**О 405 и 409.** Оба кода появились 30.08 при закрытии Д-9 (AUDIT-05). До этого
неподдерживаемый метод и нарушение ограничения БД проваливались в общий обработчик:
клиент получал `500 internal_error`, а в журнал шло «Unhandled exception», маскируя
настоящие сбои. Клиентская ошибка не должна выдаваться за серверную.
