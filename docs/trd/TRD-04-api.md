...
| Ошибки | RFC 9457 `application/problem+json`: `{type, title, status, code, detail?, errors?[{field, code}]}`; `code` — машинный ключ из §6 |
...
| Лимиты | Превышение → `429` + `Retry-After`; см. ADR-0008 §2.2 |
...
| Форма | Действия | В матрице ролей §4.4.1 |
|---|---|---|
...
| Каждый непубличный эндпоинт имеет `(form, action)` из каталога §2, согласованного с матрицей ролей §4.4.1 ТЗ-01 | Сверка таблиц (ревью) → в коде архитектурный тест FR-PERM-8 |
