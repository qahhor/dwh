...
## [2026-08-30] Пересмотр M3: второй фактор был неработоспособен и небезопасен

Полный разбор — [AUDIT-06](docs/audit/AUDIT-06-m3-auth-review.md). Коротко о главном.

### Что нашли

**Д-10. Код второго фактора никуда не отправлялся.** Между `otpCodeRepository.create(...)`
и возвратом токена не было ни одного обращения к провайдеру. Код существовал только
хешем в таблице — узнать его было неоткуда никому, включая самого пользователя.
...
Захардкоженная единица — это учётная запись администратора. Следствия оба плохие:
настоящий пользователь с 2FA войти не мог никогда (его код искали среди кодов
администратора), а в обратную сторону — при наличии активного кода у администратора
любой непустой токен вместе с угаданным шестизначным кодом открывал **сессию
администратора**.

В проде это не проявилось бы только потому, что 2FA не включена ни у кого. Проявилось бы
ровно в момент включения.

**Д-12. Каналами связи нельзя было управлять.** Таблица и репозиторий есть с V001,
эндпоинтов нет ни одного — привязать канал нечем, отправлять код некуда. Синхронизация
каталога форм с кодом как раз и пометила право `iam.profile:manage_channels` устаревшим.

### Что сделано

- **V015**: `otp_token_hash` (уникальный частичный индекс) и `purpose`. Коды, выпущенные
  до миграции, помечены использованными — они не привязаны к токену.
- Проверка ищет код **только по хешу выданного токена**; `extractUserIdFromOtpToken` удалён.
- `KauthOtpSender`: код действительно уходит в канал. Отправка синхронная — код живёт пять
  минут, очередь с повторами здесь работает против пользователя; провал доставки это отказ
  входа, а не «доставим позже».
- `KauthChannelService` + `KauthChannelController`: привязка, подтверждение владения кодом,
  список, отвязка. Неподтверждённый канал для второго фактора не используется — опечатка в
  адресе означала бы отправку кода входа постороннему.
- Нет подтверждённого канала — вход отклоняется `409 otp_channel_missing`, код не выпускается.

### Проверка

- **149 тестов, `BUILD SUCCESS`.** Новый `KauthOtpLoginIntegrationTest` — 7 сценариев,
  включая прямую регрессию на `return 1L`: выдуманный токен не открывает ничего.
- **Живой прогон 11/11:** привязка канала → код доставлен → подтверждение → включение 2FA →
  вход отдаёт `step=otp` → код доставлен → выдуманный токен отклонён (401) → настоящий
  открыл сессию того самого пользователя.
- Синхронизатор каталога при старте показал 45 живых пар вместо 44: право
  `iam.profile:manage_channels` вернулось к жизни само, как только под ним появились
  эндпоинты. Механизм Д-5 отработал в обе стороны.

### Открыто по M3

FR-AUTH-4 без экспоненциальной задержки (блокировка после 5 неудач и лимит по IP есть,
буква требования — нет); автопривязка Telegram через deep-link бота; боевой SMS-шлюз.
**Веха остаётся ЧАСТИЧНО** — по правилу приёмки закрывать её нельзя.

### О живых прогонах

Два прогона из трёх упали не на продукте, а на самом скрипте: `Where-Object` в этой
оболочке вернул все роли вместо одной (учётка получила права администратора и была
немедленно анонимизирована), а регулярка выцепила шестизначный фрагмент из `traceparent`
вместо кода. Оба раза это видно только на живом стенде — и оба раза стоило того: без
живого прогона Д-10 и Д-11 не всплыли бы вовсе.
