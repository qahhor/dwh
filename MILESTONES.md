...
### M3. Авторизация и аутентификация (AUTH) [◐ ЧАСТИЧНО] 30.08 пересмотрен целиком ([AUDIT-06](docs/audit/AUDIT-06-m3-auth-review.md)): второй фактор из неработоспособного стал рабочим (Д-10, Д-11, Д-12), Д-7 закрыт · открыто: FR-AUTH-4 без экспоненциальной задержки, боевой SMS-шлюз ждёт договор
...
| FR-AUTH-4 — экспоненциальная задержка при подборе пароля | работы; блокировка и лимит по IP уже есть (AUDIT-06 разд. 5) |
| Автопривязка Telegram через deep-link бота | бота и решения по его имени |
